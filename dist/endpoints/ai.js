...
export function createAiEndpointHandler(options = {}) {
    return async (req) => {
        ...
        try {
            ...
            return Response.json({ error: 'AI requires @puckeditor/cloud-client. This should be bundled with the plugin.' }, { status: 500 });
        }
        // 3. Check for API key
        const apiKey = process.env.PUCK_API_KEY;
        if (!apiKey) {
            return Response.json({ error: 'PUCK_API_KEY environment variable is not set' }, { status: 500 });
        }
        ...
        try {
            ...
            if (body?.config?.components) {
                ...
                const sectionComp = body.config.components['Section'];
